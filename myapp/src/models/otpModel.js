...
export const upsertOtp = async ({ email, phone, channel, code, ttl }) => {
  const now=new Date();
  const [r]=await pool.query(
    `SELECT otp_id,attempts,window_end
       FROM otps WHERE (email=? OR phone=?) AND channel=? LIMIT 1`,
    [email,phone,channel]
  );
  if(!r.length||now>r[0].window_end){
    await pool.query(
      `REPLACE INTO otps
         (otp_id,email,phone,channel,code_hash,attempts,window_end,expires_at)
       VALUES (?,?,?,?,?,1,DATE_ADD(NOW(),INTERVAL ? SECOND),
                       DATE_ADD(NOW(),INTERVAL ? SECOND))`,
      [uuid(),email,phone,channel,await bcrypt.hash(code,10),WINDOW,ttl]
    );
...
export const fetchOtp = async ({ email, phone, channel }) => {
  const [r] = await pool.query(
