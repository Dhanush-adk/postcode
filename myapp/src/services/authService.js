...
/* ───────────────────────── 1. INITIATE ─────────────────────────────── */
export const initiate = async ({ email, phone, name, type}) => {
  // const user    = await findUserByEmail(email) ?? await findUserByPhone(phone);
  let channel = phone ? 'phone' : 'email';
  logger.info(`validating OTP for ${type}`);
  if (type === 'Retailer') {
    const retailer = phone
      ? await findRetailerByPhone(phone) ?? await findRetailerByEmail(email)
      : await findRetailerByEmail(email);

      /* 1 — uniqueness guard BEFORE we send anything */
    if (channel === 'email' && email) {
      const clash = await findRetailerByEmail(email);
      if (clash && (!retailer || clash.retailer_id !== retailer.retailer_id)) {
        return {
          status: 409,
          body: {
            statusCode: 409,
    ...
    if (channel === 'phone' && phone) {
      const clash = await findRetailerByPhone(phone);
      if (clash && (!retailer || clash.retailer_id !== retailer.retailer_id)) {
        return {
          status: 409,
          body: {
    ...
    const store = phone
      ? await findStoreByPhone(phone) ?? await findStoreByEmail(email)
      : await findStoreByEmail(email);

    /* 1 — uniqueness guard BEFORE we send anything */
    if (channel === 'email' && email) {
      const clash = await findStoreByEmail(email);
      if (clash && (!store || clash.store_id !== store.store_id)) {
        return {
          status: 409,
          body: {
            statusCode: 409,
    ...
    if (channel === 'phone' && phone) {
      const clash = await findStoreByPhone(phone);
      if (clash && (!store || clash.store_id !== store.store_id)) {
        return {
          status: 409,
          body: {
    ...
    channel = decideChannel({ email, phone }, user);
  ...
  const rl   = await upsertOtp({ email, phone, channel, type, code, ttl: OTP_TTL });
...
/* ───────────────────────── 2. VERIFY ───────────────────────────────── */
export const verify = async ({ email, phone, code, name, type}) => {
  // const userPre = await findUserByEmail(email) ?? await findUserByPhone(phone);
  let channel = phone ? 'phone' : 'email';
  logger.info(`Verifying OTP for ${type}`);
  ...
  if (type === 'Customer') {
    ...
    channel = decideChannel({ email, phone }, userPre);
  }
  

  /* 1 — OTP validation */
  const otpRow = await fetchOtp({ email, phone, channel, type});
